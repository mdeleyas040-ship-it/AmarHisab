...
fun HomeLendingDialog(
    ...
    var date by remember {
        mutableStateOf(
            java.text.SimpleDateFormat("dd/MM/yyyy", java.util.Locale.getDefault()).format(
                java.util.Calendar.getInstance().apply { set(java.util.Calendar.HOUR_OF_DAY, 12) }.time
            )
        )
    }
        ...
        Card(
            ...
            Column(Modifier.fillMaxWidth().heightIn(max = 680.dp).verticalScroll(rememberScrollState())) {
                Box(Modifier.fillMaxWidth().background(primary, RoundedCornerShape(26.dp)).padding(horizontal = 18.dp, vertical = 16.dp)) {
                    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                        Surface(Modifier.size(52.dp), RoundedCornerShape(18.dp), color = onPrimary.copy(alpha = 0.12f)) {
                            Box(contentAlignment = Alignment.Center) { Icon(Icons.Default.Handshake, null, tint = onPrimary, modifier = Modifier.size(29.dp)) }
                        }
                        ...
                        Surface(onClick = onDismiss, modifier = Modifier.size(40.dp), shape = RoundedCornerShape(13.dp), color = onPrimary.copy(alpha = 0.10f)) {
                            Box(contentAlignment = Alignment.Center) { Icon(Icons.Default.Close, "বন্ধ করুন", tint = onPrimary, modifier = Modifier.size(24.dp)) }
                        }
                    }
                }
                ...
                Column(Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 14.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    Surface(Modifier.fillMaxWidth(), RoundedCornerShape(17.dp), color = primary.copy(alpha = 0.10f)) {
                        Row(Modifier.padding(horizontal = 12.dp, vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) {
                            Surface(Modifier.size(36.dp), RoundedCornerShape(12.dp), color = primary.copy(alpha = 0.12f)) {
                                Box(contentAlignment = Alignment.Center) { Icon(Icons.Default.Info, null, tint = primary, modifier = Modifier.size(20.dp)) }
                            }
                    ...
                    OutlinedTextField(
                        value = amount, onValueChange = { amount = it }, trailingContent = { MoneyShortLabel(amount) }, modifier = Modifier.fillMaxWidth(),
